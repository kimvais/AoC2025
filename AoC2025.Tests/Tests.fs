module AoC2025.Tests

open AoC2025


open FsUnit.Xunit
open Xunit


[<Fact>]
let ``day 1, part 1`` () =
    Day1.part1 "test1" () |> should equal 3L
    Day1.part1 "1" () |> should equal 1132L

[<Fact>]
let ``day1, part2`` () =
    Day1.part2 "test1" () |> should equal 6L
    Day1.part2 "1" () |> should equal 6623L

[<Fact>]
let ``day2, part1`` () =
    Day2.part1 "test2" () |> should equal 1227775554L
    Day2.part1 "2" () |> should equal 21139440284L

[<Fact>]
let ``day2 , part2`` () =
    Day2.part2 "test2" () |> should equal 4174379265L
    Day2.part2 "2" () |> should equal 38731915928L

[<Fact>]
let ``day3, part1`` () =
    Day3.part1 "test3" () |> should equal 357L
    Day3.part1 "3" () |> should equal 17435L

[<Fact>]
let ``day3, part2`` () =
    Day3.part2 "test3" () |> should equal 3121910778619L
    Day3.part2 "3" () |> should equal 172886048065379L

[<Fact>]
let ``day4, part1`` () =
    Day4.part1 "test4" () |> should equal 13L
    Day4.part1 "4" () |> should equal 1451L

[<Fact>]
let ``day4, part2`` () =
    Day4.part2 "test4" () |> should equal 43L
    Day4.part2 "4" () |> should equal 8701L

[<Fact>]
let ``day5, part1`` () =
    Day5.part1 "test5" () |> should equal 3L
    Day5.part1 "5" () |> should equal 756L

[<Fact>]
let ``day5, part2`` () =
    Day5.part2 "test5" () |> should equal 14L
    Day5.part2 "5" () |> should equal 355555479253787L

[<Fact>]
let ``day6, part1`` () =
    Day6.part1 "test6" () |> should equal 4277556L
    Day6.part1 "6" () |> should equal 6635273135233L

[<Fact>]
let ``day6, part2`` () =
    Day6.part2 "test6" () |> should equal 3263827L
    Day6.part2 "6" () |> should equal 12542543681221L

[<Fact>]
let ``day7, part1`` () =
    Day7.part1 "test7" () |> should equal 21L
    Day7.part1 "7" () |> should equal 1640L

[<Fact>]
let ``day7, part2`` () =
    Day7.part2 "test7" () |> should equal 40L
    Day7.part2 "7" () |> should equal 40999072541589L

[<Fact>]
let ``day8, part1`` () =
    Day8.part1 10 "test8" () |> should equal 40L
    Day8.part1 1000 "8" () |> should equal 103488L

[<Fact>]
let ``day8, part2`` () =
    Day8.part2 "test8" () |> should equal 25272L
    Day8.part2 "8" () |> should equal -1L

[<Fact>]
let ``day9, part1`` () =
    Day9.part1 "test9" () |> should equal 50L
    Day9.part1 "9" () |> should equal 4755278336L

[<Fact>]
let ``day9, part2`` () =
    Day9.part2 "test9" () |> should equal -1L
    Day9.part2 "9" () |> should equal -1L
